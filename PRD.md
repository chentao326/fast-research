...
> 注：以下为远期规划，Phase 1 不涉及付费功能。Phase 0 阶段的核心是验证产品价值，免费即可。

### 6.1 免费增值模型

| 层级 | 价格 | 包含 |
|------|------|------|
| 免费版 | ¥0 | 每月 10 次调研 / A+B+C+D 全格式 / 基础美化 / 导出 |
| Pro 版 | ¥29/月 | 无限调研 / 高级美化（多主题）/ 分享链接 / 历史搜索 / PDF 导出 |
| Team 版 | ¥99/月（3 人起） | Pro 全部 + 团队协作 / 共享调研库 / 飞书集成 |

### 6.2 获客策略

核心路径：**内容营销**。用 FastResearch 产出的调研报告本身就是最好的广告。

1. 产出高质量行业报告 → 发布到知乎 / 公众号 / B 站
2. 报告底部带「由 FastResearch 生成」标识 + 链接
3. 每篇报告 = 一个自然获客入口

辅助路径：
- 飞书知识库（已有订阅用户，同步发布调研文章）
- 社群分发（财富自由团等）

### 6.3 目标市场估算

| 用户群 | 中国潜在用户量 | 付费转化率（估） | 潜在付费用户 |
|--------|--------------|----------------|------------|
| 求职者（应届+跳槽） | ~2000 万/年 | 3% | 60 万 |
| 职场人（咨询/投资/产品） | ~500 万 | 5% | 25 万 |
| 内容创作者 | ~300 万 | 8% | 24 万 |
| **合计** | | | **~100 万** |

按 Pro 版 ¥29/月计，TAM ≈ ¥3.5 亿/年。实际取决于获客效率。
...
|------|------------|------------|
| AI API | ¥5/用户 | ¥15/用户 |
| Tavily Search | ¥2/用户 | ¥5/用户 |
| Vercel 托管 | ¥0.5/用户 | ¥1/用户 |
| **合计** | **~¥8/用户** | **~¥21/用户** |

Pro 版毛利 ≈ ¥8/用户/月。盈亏平衡需 ~3000 付费用户。
...
API Key 永远不出现在前端代码或网络请求中：

| Key 类型 | 存储位置 | 调用方式 |
|---------|---------|---------|
| AI API Key | 服务端环境变量 | Next.js API Route / Server Action |
| Tavily API Key | 服务端环境变量 | 同上 |
| OAuth Secret | 服务端环境变量 | NextAuth 服务端 |
| 数据库连接串 | 服务端环境变量 | Prisma / Drizzle 服务端 |

### 7.2 生成内容沙箱

| 场景 | 方案 |
|------|------|
| 在线预览 | `<iframe sandbox="allow-scripts">` 中渲染 |
| 下载 | 提供原始 HTML，用户本地打开 |
| 分享链接 | 独立子域名，隔离主站 cookie |
...
- 用户输入长度校验（< 500 字符）
- AI 生成 HTML 输出前移除 `<script>` 外链和事件处理器
- 分享链接用 UUID v4（不可猜测）
...
- 正文对比度 ≥ 4.5:1，大字 ≥ 3:1
- 键盘导航全覆盖
- 进度条等关键元素提供 aria-label

---

## 八、设计原则（Skill 版→Web App 版继承）

### 8.1 「AI 做 dirty work，人做 thinking」
...
- [ ] Next.js 项目初始化 + Tailwind + shadcn/ui + Zustand
- [ ] 全局 UI 框架（五步进度条 + 常驻问题栏）
- [ ] Step 1：话题输入 + 示例问题引导 + AI 扩展
- [ ] Tavily Search 集成
- [ ] Step 2-3：搜索 pipeline + 框架生成 + 深度聊天
- [ ] Step 4：四种输出 + 流式生成 + 美化自动触发
- [ ] Step 5：并排对比面板
- [ ] 异常流降级（搜索失败/AI 超时/断点续传）
- [ ] 调研历史（Vercel Postgres + localStorage 持久化）
- [ ] Vercel 部署
